...
Sobreimpulso & \% & Modelo FODT identificado & Control PID \\
Error en estado estacionario & m/s & Modelo FODT identificado & Control PID \\
...
\begin{tablenotes}
\footnotesize
\item[a] Las métricas del controlador PID se calculan mediante simulación sobre el modelo FODT identificado con las ganancias que operan en el robot, no por medición directa sobre el hardware.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
\textbf{Procedimiento:} se envió una velocidad lineal constante de $0.13$ m/s durante el tiempo correspondiente a $2.00$ m teóricos, con cinco repeticiones; se midió la distancia real con cinta métrica y se comparó con \texttt{/odom}. El error porcentual se define según el entorno. En simulación, donde la odometría del plugin de Gazebo coincide con la verdad de terreno (\textit{ground truth}), el indicador cuantifica el error de seguimiento de consigna (diferencia entre la distancia comandada y la ejecutada por el controlador):

\begin{equation}
e_{\text{seg}} = \frac{|d_{\text{teórica}} - d_{\text{odom}}|}{d_{\text{teórica}}} \times 100 \quad [\%],
\qquad d_{\text{odom}} = d_{\text{GT}}
\label{eq:error_seg}
\end{equation}

\noindent
En hardware real, donde $d_{\text{real}}$ es la distancia física medida con cinta, el indicador cuantifica el error de odometría propiamente tal (discrepancia entre la odometría de encoders y la realidad):

\begin{equation}
e_{\text{odom}} = \frac{|d_{\text{odom}} - d_{\text{real}}|}{d_{\text{real}}} \times 100 \quad [\%]
\label{eq:error_odom}
\end{equation}
...
\begin{tablenotes}
\footnotesize
\item[a] En Gazebo la odometría coincide con el \textit{ground truth}; se reporta una sola columna. Error según la Ec.~\ref{eq:error_seg}.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
\begin{tablenotes}
\footnotesize
\item El error cuantifica la discrepancia entre la odometría de encoders ($d_{\text{odom}}$) y la distancia física medida con cinta ($d_{\text{real}}$), según la Ec.~\ref{eq:error_odom}.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
\begin{tabular}{ccccc}
\toprule
\textbf{Prueba} & \textbf{Teórico (\textdegree)} & \textbf{Odom. (\textdegree)} & \textbf{Real GT (\textdegree)} & \textbf{Desv. (\textdegree)}\tnote{a} \\
\midrule
1 & 360.0 & 335.86 & 335.86 & 24.14 \\
2 & 360.0 & 347.40 & 347.40 & 12.60 \\
3 & 360.0 & 346.62 & 346.62 & 13.38 \\
4 & 360.0 & 347.40 & 347.40 & 12.60 \\
5 & 360.0 & 337.41 & 337.41 & 22.59 \\
\midrule
\multicolumn{4}{r}{\textbf{Desviación media}} & \textbf{17.1} \\
\bottomrule
\end{tabular}
\begin{tablenotes}
\footnotesize
\item[a] Desviación $= 360^\circ$ (comandado) $-$ GT; cuantifica el seguimiento de consigna (sub-rotación), no la deriva de la odometría. El ángulo teórico corresponde al tiempo de comando.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
\begin{tablenotes}
\scriptsize
\item[a] Ganancias en forma estándar continua, para comparar con Lambda. Las del firmware se obtienen como $k_p = K_p^{\text{fw}}/f_u$ y $k_d = K_d^{\text{fw}}/f_u$, con el factor de conversión de unidades $f_u = \texttt{PWM\_NOMINAL}\cdot K_o/\texttt{MPS\_TO\_TICKS} = 143\cdot 50/313.56 \approx 22.80$; el término integral incorpora además el período de muestreo del lazo: $k_i = (K_i^{\text{fw}}/f_u)\cdot f_{\text{PID}}$.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
\item[b] No estrictamente comparable: en Gazebo el error refleja el seguimiento de consigna (odometría $=$ \textit{ground truth}, Ec.~\ref{eq:error_seg}); en hardware refleja la discrepancia entre encoders y la medición física (Ec.~\ref{eq:error_odom}).
\item[c] Tampoco comparable directamente: en simulación cuantifica el seguimiento de consigna (el robot recorre $\approx 343^\circ$ de los $360^\circ$ comandados por dinámica y discretización); en hardware cuantifica la deriva de la odometría respecto de la orientación física. Se reportan en sus unidades nativas (grados) y no deben restarse.
\end{tablenotes}
\end{threeparttable}
\end{table}
