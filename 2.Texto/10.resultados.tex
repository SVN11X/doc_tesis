...
\begin{utemMultiFigurePropia}{Secuencia de navegación en simulación.}{fig:sample_nav_sim_v1}
...
\begin{tablenotes}
\footnotesize
\item[a] El PSO es estocástico: $K_m$ y $\tau$ son consistentes entre corridas, mientras que el retardo $L$ varía. Se reportan los valores de las dos corridas empleadas, respectivamente, para la sintonía PSO y para la comparación con Lambda.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
\begin{tablenotes}
\scriptsize
\item[a] Las ganancias del firmware se expresan en la escala continua estándar para compararlas con Lambda: los términos proporcional y derivativo se escalan por $f_\lambda \approx 22.80$, y el integral incorpora además el período de muestreo del lazo.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
La Tabla~\ref{tab:fallos_nav} clasifica los modos de fallo previstos y la acción de recuperación del árbol de comportamiento que los atiende; la columna de resultado se completará con la tasa de recuperación observada.
...
\small
\begin{tabular}{p{3.2cm} p{3.5cm} p{4cm} p{2.4cm}}
\toprule
...
La exploración se evalúa colocando el robot en una esquina y lanzando \texttt{explore\_server}, registrando el número de metas generadas y alcanzadas, la cobertura y las intervenciones manuales. Al igual que la navegación, requiere un área mayor que la disponible, por lo que las Tablas~\ref{tab:explore_sim} y~\ref{tab:explore_real} quedan planificadas.
...
La transferencia al hardware requirió: fijar \texttt{use\_sim\_time:=false}; activar las reglas \texttt{udev} de nombres persistentes (\texttt{/dev/motor\_controller}, \texttt{/dev/rplidar}); reducir la velocidad lineal máxima de $0.26$ a $0.13$ m/s y la frecuencia del controlador DWB de 20 a 10 Hz; y habilitar el retroceso (\texttt{min\_vel\_x} $= -0.13$ m/s). El detalle completo de las diferencias de configuración entre entornos se documenta en el Capítulo~\ref{chap:app_control_nav}.

% ===========================================================================
\section{Viabilidad en la Raspberry Pi 4: recursos y autonomía}
\label{sec:recursos_computacionales}

Durante las sesiones de SLAM y navegación superiores a 10~min de operación continua no se observaron caídas de tensión ni sobrecalentamiento en el riel de 5~V, comportamiento consistente con el margen del convertidor frente al presupuesto de potencia (Tabla~\ref{tab:power_budget_filled}); la etapa de potencia opera con reserva suficiente para la carga del prototipo y para la incorporación futura de sensores. La caracterización cuantitativa del uso de recursos de cómputo (CPU, RAM, temperatura del SoC, latencia de \texttt{/scan}) y de la autonomía energética queda pendiente de medición sistemática.
...
La validación de la navegación autónoma y la exploración de fronteras quedó limitada por la extensión del recinto físico disponible, que no admite los escenarios definidos (meta a $\approx 3$ m, pasillo). El protocolo, las métricas y la infraestructura de pruebas están definidos, de modo que estas evaluaciones pueden completarse en un entorno de mayor superficie para cerrar cuantitativamente la validación del sistema integrado.