...
\begin{algorithm}[H]
\caption{PSO ejecutado por \texttt{pyPso.py}}
\label{alg:pso_nucleo}
\begin{algorithmic}[1]
\Require Función de costo $J$; partículas $N_p$; generaciones $g_{\max}$; $c_1=c_2=2.05$; $\chi=0.7298$; modo ID o PID.
...
\While{la exploración esté activa}
    ...
    \For{cada frontera $F\in\mathcal{F}$}
        \State Calcular $p_F^{\mathrm{impl}}$ mediante la Ec.~\eqref{eq:centroide_frontera_impl}.
        \State Calcular $U(F)$ mediante la Ec.~\eqref{eq:frontier_cost}.
    \EndFor
