...
La comunicación con el firmware se realiza mediante el plugin
\texttt{DiffDriveArduino}, una interfaz de sistema de \texttt{ros2\_control}
cuyo ciclo de lectura--actualización--escritura, formalizado en el
Alg.~\ref{alg:ros2control_cycle}, se programa nominalmente con
$f_{\mathrm{ctrl}}=30$~Hz mediante \texttt{controller\_manager.update\_rate}.
...
representa el truncamiento hacia cero producido al convertir la expresión en punto flotante al argumento entero de la interfaz de envío de comandos de motor Alg.~\ref{alg:ros2control_cycle}); y $m_{j,\mathrm{ROS}}^{\mathrm{cmd}}$ se expresa en ticks/frame. La división
utiliza el parámetro independiente \texttt{loop\_rate} de la interfaz, denotado
...
En la fase de lectura, la transformación desde los conteos acumulados hasta los estados de rueda sigue la cadena:
...
resume, a nivel macro, en el Alg.~\ref{alg:odometria_diferencial}: se
...
\begin{algorithm}[H]
\caption{Transformación entre encoders, estados y consignas}
\label{alg:odometria_diferencial}
\begin{algorithmic}[1]
\Require Conteos $c_L[k],c_R[k]$; estados previos
$\phi_L[k-1],\phi_R[k-1]$; resolución $N_e$; intervalo
$\Delta t_k$; consignas $\omega_L^{\mathrm{cmd}},
\omega_R^{\mathrm{cmd}}$; frecuencia $f_{\mathrm{loop}}$.
\Ensure Estados de rueda y referencias enteras en ticks/frame.

\For{$j\in\{L,R\}$}
\State $(\phi_j[k],\widehat{\omega}_j[k])
\gets
\Call{EstimarEstadoRueda}
{c_j[k],\phi_j[k-1],N_e,\Delta t_k}$
\EndFor

\State \Call{ExportarEstados}
{$\widehat{\omega}_L,\phi_L,\widehat{\omega}_R,\phi_R$}

\For{$j\in\{L,R\}$}
\State $m_j^{\mathrm{cmd}}
\gets
\Call{ConvertirConsigna}
{\omega_j^{\mathrm{cmd}},N_e,f_{\mathrm{loop}}}$
\EndFor

\State \Call{EnviarComando}
{\texttt{m},$m_L^{\mathrm{cmd}}$,$m_R^{\mathrm{cmd}}$}
\end{algorithmic}
\end{algorithm}

La operación \Call{EstimarEstadoRueda} corresponde a las
Ecs.~\eqref{eq:posicion_rueda_encoder} y~\eqref{eq:w_ruedas}, mientras que
\Call{ConvertirConsigna} implementa la Ec.~\eqref{eq:mps_to_ticks}. El orden exacto de ejecución, las validaciones seriales y la inicialización
temporal se formalizan en el Alg.~\ref{alg:ros2control_cycle}; la
definición por casos del intervalo $\Delta t_k$ se conserva en el
...
\While{la interfaz esté iniciada}
    ...
    \For{$j\in\{L,R\}$}
        \State $\phi_j[k]\gets\delta\phi\,c_j[k]$
        \State $\widehat{\omega}_j[k]\gets\dfrac{\phi_j[k]-\phi_j[k-1]}{\Delta t_k}$
    \EndFor
    ...
    \EndIf
    \For{$j\in\{L,R\}$}
        \State $m_j^{\mathrm{cmd}}\gets\operatorname{trunc}_{0}\!\left(\dfrac{N_e}{2\pi f_{\mathrm{loop}}}\,\omega_j^{\mathrm{cmd}}\right)$
    \EndFor
...
la estructura macro del Alg.~\ref{alg:pid_macro}.

\begin{algorithm}[H]
\caption{Estructura macro del control PID de velocidad por rueda}
\label{alg:pid_macro}
\begin{algorithmic}[1]
\Require Referencias $m_L^{\mathrm{cmd}},m_R^{\mathrm{cmd}}$;
estructuras PID $p_L,p_R$.
\Ensure Salidas PWM $u_L,u_R$.

\State $(c_L[k],c_R[k])\gets\Call{LeerEncoders}{}$
\State $p_R\gets\Call{doPID}{p_R,1}$
\State $p_L\gets\Call{doPID}{p_L,2}$
\State \Call{setMotorSpeeds}{$p_L.output,p_R.output$}
\end{algorithmic}
\end{algorithm}

La implementación interna de \Call{doPID} se presenta en el
Alg.~\ref{alg:pid_firmware_ley}.
...
reinicios. La secuencia exacta de lectura, actualización de estados,
saturación y aplicación de las salidas, incluidos los nombres de las
funciones del firmware involucradas, se formaliza en los
Algoritmos~\ref{alg:pid_firmware} y~\ref{alg:pid_firmware_ley}: el primero
describe el ciclo de actualización y la lógica de reinicio, y el segundo,
la ley de control discreta aplicada a cada rueda.
...
\Procedure{updatePID}{}
    ...
    \If{\texttt{moving}$=0$}
        ...
        \State \Return
    \EndIf
    \State \Call{doPID}{PID derecho,$1$} \Comment{Alg.~\ref{alg:pid_firmware_ley}}
...
\begin{algorithm}[H]
\caption{Ley de control PID discreta por rueda ejecutada por el firmware}
\label{alg:pid_firmware_ley}
\begin{algorithmic}[1]
\Require Estructura $p$ de la rueda; zona muerta $e_{db}$; ganancias $K_p,K_i,K_d$; $K_o$; \texttt{MAX\_PWM}$=255$.
\Ensure $p.output$ saturado en $[-255,255]$ y estados $p.ITerm$, $p.PrevInput$, $p.PrevEnc$ actualizados.
\Procedure{doPID}{$p,e_{db}$}
    \State $m\gets\operatorname{int}(p.Encoder-p.PrevEnc)$
    \State $e\gets\operatorname{trunc}_{0}(p.TargetTicksPerFrame-m)$
    \If{$|e|\leq e_{db}$}
        \State $e\gets0$
    \EndIf
    \State $\Delta u\gets\operatorname{trunc}_{0}\!\left(\dfrac{K_p e-K_d(m-p.PrevInput)+p.ITerm}{K_o}\right)$
    \State $p.PrevEnc\gets p.Encoder$
    \State $u^{\mathrm{raw}}\gets p.output+\Delta u$
    \If{$u^{\mathrm{raw}}\geq255$}
        \State $u\gets255$
    \ElsIf{$u^{\mathrm{raw}}\leq-255$}
        \State $u\gets-255$
    \Else
        \State $u\gets u^{\mathrm{raw}}$
        \State $p.ITerm\gets\operatorname{trunc}_{0}(p.ITerm+K_i e)$
    \EndIf
    \State $p.output\gets u$
    \State $p.PrevInput\gets m$
\EndProcedure
\end{algorithmic}
\end{algorithm}
...
\For{$g=1,\ldots,g_{\max}$}
    \State $w_{\mathrm{in}}^{g}\gets\dfrac{g_{\max}-g}{g_{\max}}$
    \For{$i=1,\ldots,N_p$}
        ...
        \State $\xi_i\gets\max(\xi_i+V_i,10^{-4})$
        \If{modo ID}
            \State $\theta_i^{\mathrm{eval}}\gets(0.01\xi_{i,1},2\xi_{i,2},0.5\xi_{i,3})$
        \Else
            \State $\xi_i^{\mathrm{eval}}\gets\min(1,\max(\xi_i,10^{-4}))$
...
\EndFor
\If{modo ID}
    \State $\theta^{\mathrm{out}}\gets(0.01G_1,2G_2,0.5G_3)$
\Else
...
La operación \Call{ActualizarVelocidadPSO} implementa la
Ec.~\eqref{eq:pso_vel_marco_teorico}. El orden $\{L,R\}$ del bucle de sintonización reproduce el orden en que los
...
La salida de la ejecución utilizada como punto de partida, incluidos los parámetros FODT identificados y las ganancias sugeridas por PSO, se conserva en la Tabla~\ref{tab:pso_resultados} del \ref{anx:resultados_pso_ejecucion}. Al no fijarse una semilla pseudoaleatoria, los valores pueden variar entre ejecuciones. Además, con $T_{\mathrm{cap}}=0.1$~s, los retardos obtenidos en esa ejecución son inferiores a un período de captura y no producen un desplazamiento temporal efectivo en el modelo discreto.
...
queda limitada posteriormente por el controlador diferencial. La Tabla~\ref{tab:nav2_componentes_ch9} resume los componentes de Nav2 utilizados y la función de cada uno.
