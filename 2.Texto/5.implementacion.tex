...
El flujo completo entre \gls{ROS2}, la interfaz de hardware y el firmware se formaliza en el Alg.~\ref{alg:ros2control_cycle}. Las conversiones de estado y consigna corresponden a las Ecs.~\eqref{eq:posicion_rueda_encoder}, \eqref{eq:w_ruedas} y~\eqref{eq:mps_to_ticks}; la definición por casos de $\Delta t_k$ se conserva en el \ref{anx:implementacion_diffdrive}. El orden exacto de ejecución, las validaciones seriales y la inicialización
...
Alg.~\ref{alg:pid_firmware}, este describe el ciclo de actualización y la lógica de reinicio
...
La secuencia exacta de lectura, actualización de estados, saturación y aplicación de las salidas se formaliza en el Alg.~\ref{alg:pid_firmware}; la ley discreta ejecutada por cada llamada a \texttt{doPID()} se detalla en las Ecs.~\eqref{eq:medicion_pid_entera}--\eqref{eq:integrador_condicional}.
