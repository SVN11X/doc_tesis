...
Algoritmo~\ref{alg:ros2control_cycle}, se programa nominalmente con
...
representa el truncamiento hacia cero producido al convertir la expresión en punto flotante al argumento entero de la interfaz de envío de comandos de motor Algoritmo~\ref{alg:ros2control_cycle}); y $m_{j,\mathrm{ROS}}^{\mathrm{cmd}}$ se expresa en ticks/frame. La división
...
temporal se formalizan en el Algoritmo~\ref{alg:ros2control_cycle}; la
...
\begin{algorithmic}[1]
\small
\Require Comandos de rueda $\omega_L^{\mathrm{cmd}},\omega_R^{\mathrm{cmd}}$ [rad/s]; enlace serial; $N_e$; frecuencia nominal \texttt{loop\_rate}$=30$.
...
\State Enviar \texttt{m} seguido de $m_{L,\mathrm{ROS}}^{\mathrm{cmd}}$ y $m_{R,\mathrm{ROS}}^{\mathrm{cmd}}$.
\EndWhile
\end{algorithmic}
...
funciones del firmware involucradas, se formaliza en el
Algoritmo~\ref{alg:pid_firmware}.
...
\Statex
\Procedure{doPID}{$p,e_{\mathrm{db}}$}
\State $m^{\mathrm{int}}\gets\operatorname{int}(p.Encoder-p.PrevEnc)$.
\State $e^{\mathrm{long}}\gets\operatorname{trunc}_0(p.TargetTicksPerFrame-m^{\mathrm{int}})$.
\If{$|e^{\mathrm{long}}|\leq e_{\mathrm{db}}$}
\State $e^{\mathrm{long}}\gets0$.
\EndIf
\State $\Delta u^{\mathrm{long}}\gets\operatorname{trunc}_0\!\left[(K_p e^{\mathrm{long}}-K_d(m^{\mathrm{int}}-p.PrevInput)+p.ITerm)/K_o\right]$.
\State $p.PrevEnc\gets p.Encoder$.
\State $u^{\mathrm{raw}}\gets p.output+\Delta u^{\mathrm{long}}$.
...
\State $p.ITerm\gets\operatorname{trunc}_0(p.ITerm+K_i e^{\mathrm{long}})$.
\EndIf
\State $p.output\gets u$; \quad $p.PrevInput\gets m^{\mathrm{int}}$.
\EndProcedure
\end{algorithmic}
...
flujo completo se formaliza en el Algoritmo~\ref{alg:pso_nucleo}; las
...
El orden $\{L,R\}$ del bucle de sintonización reproduce el orden en que los
scripts recorren las ruedas, formalizado en el Algoritmo~\ref{alg:sintonizacion}.
...
Algoritmo~\ref{alg:captura_lazo_abierto}; la ecuación de conversión de
...
\begin{algorithmic}[1]
\small
\Require Enlace serial al \gls{Arduino}; PWM nominal de la Tabla~\ref{tab:pso_config}; período de captura $T_{\mathrm{cap}}=0.1$~s e instantes de muestreo $t_k=k\,T_{\mathrm{cap}}$ [s]; duración $t_f=10$~s; instante de escalón $t_{\mathrm{step}}=1.0$~s; resolución del encoder $N_e$; radio de rueda $r$.
\Ensure Series por rueda $\bigl(u_j[k],v_j^{\mathrm{med}}[k]\bigr)$, $j\in\{R,L\}$, para la identificación FODT de la Etapa~1 del Alg.~\ref{alg:sintonizacion}.
...
\State $u_j[k]\gets \mathrm{PWM}_{\mathrm{nom}}$, para $j\in\{R,L\}$
...
\State Calcular $v_j^{\mathrm{med}}[k]$ mediante la Ec.~\ref{eq:vel_lazo_abierto}.
\State Registrar $\bigl(u_j[k],v_j^{\mathrm{med}}[k]\bigr)$.
\State $c_j[k-1]\gets c_j[k]$
\EndFor
\State Calcular el tiempo real consumido por el ciclo.
\If{el tiempo consumido es menor que $T_{\mathrm{cap}}$}
\State Dormir únicamente el remanente hasta $T_{\mathrm{cap}}$.
\EndIf
\State $k\gets k+1$
\EndWhile
\Statex
...
$\bigl(u_j[k],v_j^{\mathrm{med}}[k]\bigr)$ para identificar el modelo FODT.
\end{algorithmic}
