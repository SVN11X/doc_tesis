...
\begin{algorithmic}[1]
\Require Conteos $c_L[k],c_R[k]$; estados previos $\phi_L[k-1],\phi_R[k-1]$; resolución $N_e$; intervalo $\Delta t_k$; consignas $\omega_L^{\mathrm{cmd}},\omega_R^{\mathrm{cmd}}$; frecuencia $f_{\mathrm{loop}}$.
\Ensure Estados de rueda y referencias enteras en ticks/frame.
\For{$j\in\{L,R\}$}
    \State $\phi_j[k]\gets \dfrac{2\pi}{N_e}\,c_j[k]$
    \State $\widehat{\omega}_j[k]\gets \dfrac{\phi_j[k]-\phi_j[k-1]}{\Delta t_k}$
\EndFor
\State Exportar $(\widehat{\omega}_L,\phi_L,\widehat{\omega}_R,\phi_R)$ a \texttt{ros2\_control}.
\For{$j\in\{L,R\}$}
...
\label{alg:pid_macro}
\begin{algorithmic}[1]
\Require Referencias $m_j^{\mathrm{cmd}}$; conteos $c_j$; ganancias $(K_{p,j},K_{d,j},K_{i,j},K_o)$; bandas muertas $e_{db,j}$.
\Ensure Salidas PWM $u_L,u_R$.
\For{$j\in\{R,L\}$, cada $33$~ms}
    \State $m_j[k]\gets c_j[k]-c_j[k-1]$
    \State $e_j[k]\gets m_j^{\mathrm{cmd}}[k]-m_j[k]$
    \If{$|e_j[k]|\leq e_{db,j}$}
        \State $e_j[k]\gets0$
    \EndIf
    \State Calcular $\Delta u_j[k]$ mediante la Ec.~\eqref{eq:pid_incremento}.
    \State $u_j^{\mathrm{raw}}[k]\gets u_j[k-1]+\Delta u_j[k]$
    \State $u_j[k]\gets\operatorname{sat}_{[-255,255]}\!\left(u_j^{\mathrm{raw}}[k]\right)$
    \If{$-255<u_j^{\mathrm{raw}}[k]<255$}
        \State Actualizar el estado integral $I_j[k]$.
    \EndIf
    \State Aplicar $u_j[k]$ al motor correspondiente.
\EndFor
\end{algorithmic}
\end{algorithm}
...
\For{$g=1,\ldots,g_{\max}$}
    ...
    \For{$i=1,\ldots,N_p$}
        ...
        \State Actualizar $V_i$ mediante la Ec.~\eqref{eq:pso_vel_marco_teorico}.
