...
El firmware utiliza las mismas constantes físicas definidas para el modelo cinemático y el gemelo digital: el diámetro de rueda $D=\texttt{WHEEL\_DIAMETER}=0.067$~m (equivalente a $D=2r$) y $N_e=\texttt{ENC\_TICKS\_PER\_REV}=1980$ cuentas/vuelta. El lazo de control opera a $f_{PID}=\texttt{PID\_RATE}=30$~Hz, por lo que el período de control es $T_s=1/f_{PID}\approx0.0333$~s.
...
donde $N_e$ es la resolución del encoder [cuentas/vuelta], $D$ el diámetro de rueda [m] y $f_{PID}$ la frecuencia del lazo de control [Hz]. Este factor corresponde a la vía de comando en m/s (\texttt{SET\_VEL\_MPS}); en la operación con \texttt{ros2\_control}, el plugin \texttt{DiffDriveArduino} envía directamente la referencia en \textit{ticks/frame} (Capítulo~\ref{chap:robot_real}).
...
donde $e_j[k]$ es el error [ticks/frame], $m_j^{ref}[k]$ la referencia [ticks/frame] y $m_j[k]$ la medición de velocidad de rueda, obtenida como la diferencia de cuentas del encoder entre dos ciclos consecutivos del lazo de control [ticks/frame].
...
donde $\Delta\texttt{enc}_j[k]$ son las cuentas registradas en el intervalo $T_{cap}$ [cuentas], $T_{cap}=0.1$~s el período de captura, $D=0.067$~m el diámetro de rueda y $N_e=1980$ cuentas/vuelta la resolución del encoder. El término $\pi D$ corresponde a la circunferencia de la rueda [m]. La variable $v_j[k]$ representa velocidad lineal de rueda, no velocidad lineal del robot.
...
donde $y[k]$ es la salida estimada del modelo (velocidad de rueda) [m/s], $u[k]$ la entrada PWM aplicada en lazo abierto [PWM], $K_m$ la ganancia estática [m/s/PWM], $\tau$ la constante de tiempo [s] y $n_d=\lfloor L_d/T_{cap}\rfloor$ el retardo expresado en número de muestras (con $L_d$ el retardo puro [s]). Luego, un segundo PSO buscó las ganancias $(K_p,K_d,K_i)$ que minimizan una función de costo basada en error cuadrático medio y sobreimpulso. El procedimiento completo de sintonización se resume en el Algoritmo~\ref{alg:sintonizacion}.
...
\For{$j \in \{R,L\}$}
    ...
    \While{$g < g_{max}$}
        \State Simular FODT por Euler con $T_{cap}$ (Ec.~\ref{eq:euler_fodt}) para cada partícula
