...
Los datos disponibles no permiten atribuir experimentalmente el 7.18\% a un
mecanismo individual. Efectos no representados completamente en simulación,
como fricción, zona muerta del accionamiento, deslizamiento, cuantización y
dinámica transitoria, pueden contribuir a la diferencia observada, pero no
fueron aislados mediante ensayos independientes. Por ello, el valor se
interpreta como una medida agregada del seguimiento físico del prototipo y no
como una estimación de alguno de esos efectos por separado.
...
se calculan sobre esa misma respuesta medida.

Los indicadores de la Tabla~\ref{tab:pid_performance} se calcularon
directamente sobre la respuesta experimental en lazo cerrado de la
Fig.~\ref{fig:respuesta_escalon_final}, con la referencia de 0.13~m/s. La
comparación ilustrativa con el modelo FOPDT y la sintonía Lambda se conserva
en el \ref{anx:lambda_detalle}.
...
De las 60 metas generadas, 54 finalizaron con estado abortado, una con éxito y
cinco quedaron sin estado terminal al finalizar sus respectivos registros. Por
tanto, la fracción de metas exitosas respecto del total generado fue de
$1/60\approx1.7\%$, mientras que entre las 55 metas con resultado terminal fue
de $1/55\approx1.8\%$. Estos valores muestran que la cadena
explorador--Nav2 se ejecutó, pero también evidencian un desempeño operacional
limitado para completar las metas seleccionadas en el entorno utilizado.

Los once mensajes de lista de fronteras agotada no representan once abortos ni
deben compararse directamente con las 54 metas abortadas. En el código
...
iteraciones se refiere a la generación de un mapa válido. La validación
cuantitativa de navegación dirigida con metas fijas, distinta de las metas que
genera el explorador, permanece pendiente. La Tabla~\ref{tab:estado_validacion} sintetiza el estado de validación alcanzado por cada subsistema.
...
Control PID & Validado con datos experimentales & Respuesta e indicadores de la Tabla~\ref{tab:pid_performance} medidos sobre la captura real en lazo cerrado \\
