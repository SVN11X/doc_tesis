...
Esta métrica no es un error odométrico angular aislado. Mide cuánto se aparta el giro acumulado de \texttt{/odom} al final de un ensayo de duración fija respecto de la referencia nominal de 360~grados. Lo que domina el valor es la combinación de la duración del ensayo y el límite de velocidad angular, no la deriva odométrica. Por eso la desviación en simulación supera a la de hardware, ya que la velocidad angular efectiva difiere entre ambos backends. Un giro comandado a un ángulo fijo y controlado, planteado como trabajo futuro, entregaría un error angular directo.
...
La ganancia integral difiere entre ruedas, con $K_{i,L}=0.075$ y $K_{i,R}=0.001$. Esta diferencia responde a la banda muerta asimétrica del firmware, de 2~ticks/trama en la rueda izquierda y 1 en la derecha. Dentro de la banda muerta el error se fija en cero, de modo que no se acumula acción integral. La rueda izquierda ignora así un rango de error mayor y requiere una ganancia integral más alta para eliminar el error estacionario residual una vez fuera de esa banda. Este comportamiento es coherente con la respuesta obtenida con las ganancias sugeridas por PSO, en la que la rueda izquierda mantenía un error estacionario bajo la referencia (Fig.~\ref{fig:respuesta_escalon_pso}). El refinamiento experimental fijó la ganancia integral izquierda para corregir ese sesgo.

El PSO entregó un punto de partida heurístico para las tres ganancias. El refinamiento experimental ajustó ese punto de partida, de forma moderada en $K_p$ y $K_d$ y de forma más marcada en $K_i$, hasta lograr el seguimiento de la referencia sobre el robot real. Por tanto, el aporte del PSO fue el de una semilla de búsqueda y no el de un ajuste final validado por sí solo.
...
Este patrón es coherente con el recinto reducido y el radio de inflación de 0.15~m, que dejan poco margen para completar rutas hacia fronteras cercanas a las paredes. El comportamiento se corrobora con once avisos de lista de fronteras agotada emitidos por el nodo explorador, que vacía y reintenta la lista negra. En la implementación empleada una frontera entra en la lista negra únicamente cuando su meta retorna aborto, por lo que esos avisos confirman abortos reales.

En conjunto, estos registros demuestran el mecanismo autónomo completo. El explorador elige una frontera, envía la meta, Nav2 la ejecuta y el mapa crece con la información nueva del LIDAR. La baja tasa de metas completadas queda documentada como una limitación cuantificada del entorno de pruebas y no como una falla del mecanismo.
...
Nav2 & Verificada funcionalmente; resultado de metas cuantificado & Distribución de resultados de 55 metas terminales de exploración (Tabla~\ref{tab:anx_explore_metas}); protocolo de navegación dirigida con metas fijas pendiente \\
Exploración & Validada a nivel de meta & 60 metas autónomas generadas en 5 iteraciones, 54 abortos y 1 éxito; cobertura y área cuantificadas \\
\bottomrule
\end{tabular}
\end{table}
