...
La navegación con Nav2 hacia las metas generadas automáticamente por el
explorador se evalúa funcionalmente y mediante el resultado terminal de dichas
metas, mientras que la exploración se caracteriza además mediante cobertura y
área mapeada. La exploración por fronteras se ejecutó de forma autónoma en cinco iteraciones válidas. El nodo explorador seleccionó fronteras y despachó metas de navegación a Nav2 mediante la acción \texttt{NavigateToPose}. Los contadores automáticos de la sesión informaban cero metas generadas porque ninguna de sus dos fuentes del lado de la meta produjo coincidencias durante los ensayos. Sus contadores de fallo y de éxito, en cambio, se incrementan una vez por cada mensaje del flujo de estado, sin deduplicar identificadores de meta, de modo que no expresan números de metas únicas. El detalle de este comportamiento se documenta en el \ref{anx:resultados_metodologia}. Para obtener una medida fiable, cada meta se reconstruyó por su identificador único a partir del flujo de estado de la acción registrado en cada sesión, según la Tabla~\ref{tab:anx_explore_metas} del \ref{anx:protocolo_resultados}.
