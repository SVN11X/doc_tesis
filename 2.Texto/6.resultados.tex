...
Este capítulo presenta los resultados obtenidos durante la validación del prototipo de robot móvil diferencial. La odometría diferencial y la sintonización PID se evaluaron en simulación y en hardware real; el mapeo con \texttt{slam\_toolbox}, la navegación autónoma y la exploración de fronteras se evaluaron sobre el hardware real, en el entorno físico disponible. Los resultados complementarios de exploración y mapeo se presentan en el \ref{anx:protocolo_resultados}. El equipamiento de la estación de trabajo y las versiones de sistema
operativo, middleware y paquetes con que se ejecutaron todos los ensayos se
consolidan en el \ref{anx:workspace}. El entorno físico en que se realizaron las pruebas sobre hardware real se muestra en la Fig.~\ref{fig:entorno_experimental}.
...
\noindent donde $D_{\theta,360}$ es la desviación angular normalizada [\%],
...
\end{tabular}
\vspace{3pt}
\parbox{0.85\linewidth}{\footnotesize $^{*}$ En simulación se calcula con la distancia de odometría, porque la pose verdadera de Gazebo Classic no quedó registrada.}
\end{table}
...
\begin{table}[htbp]
\centering
\caption{Desviación angular respecto de la referencia nominal de $360^\circ$ (5 repeticiones por entorno).}
...
Desviación angular normalizada $D_{\theta,360}$ & $4.74\%$ & $3.24\%$ \\
...
\section{Evaluación del control de velocidad}
...
las ganancias definitivas. La Fig.~\ref{fig:respuesta_escalon_final} presenta la respuesta obtenida con las ganancias definitivas, bajo el mismo protocolo y la misma referencia.
...
El índice de cobertura libre se calcula sobre la imagen de ocupación completa, cuyos límites no están anclados al recinto físico. Por eso es un indicador interno relativo y no una fracción absoluta del área del recinto. Esto explica que el área libre mapeada se mantenga casi constante entre iteraciones mientras el índice de cobertura varía de forma amplia, ya que este último depende de cuántas celdas desconocidas quedan dentro de la imagen. Por su mayor estabilidad, el área libre mapeada y el término de cada ejecución por ausencia de fronteras (Sec.~\ref{sec:resultados_nav2}) se toman como evidencia principal de la exploración, y el índice de cobertura como indicador complementario.

\section{Navegación autónoma y exploración}
\label{sec:resultados_nav2}

Los resultados de navegación y exploración se analizan en dos niveles
complementarios. El primero corresponde al resultado de cada ejecución completa
de exploración, considerando si el robot operó de forma autónoma, se desplazó
por el entorno, incorporó nuevas observaciones y produjo un mapa válido. El
segundo corresponde al resultado de las metas individuales generadas durante
dicha exploración.

Esta distinción permite evaluar por separado el funcionamiento global de la
exploración y la capacidad del sistema de navegación para completar cada una de
las metas seleccionadas. Por tanto, el resultado de una meta individual no se
interpreta directamente como el resultado de toda la ejecución de exploración.

La exploración autónoma produjo un mapa válido en cinco de las seis ejecuciones
experimentales. En estas cinco ejecuciones, el índice de cobertura libre varió
entre 53.4\% y 82.9\%, con una media de 62.2\%, mientras que el área libre
mapeada se mantuvo entre 3.478 y 3.540~m$^2$, con una media de
3.50~m$^2$.

Las cinco ejecuciones válidas terminaron de forma automática, sin intervención
del operador. En cada una, el nodo \texttt{explore\_server} informó que no
quedaban fronteras en el mapa de costos, detuvo la exploración y solicitó
cancelar la meta en curso. El script de ensayo detecta ese aviso, guarda el
mapa y cierra el registro. La iteración~2, excluida del análisis, no llegó a ese
término. Las intervenciones manuales fueron nulas en las cinco ejecuciones
válidas.

En total se generaron 60 metas de navegación durante las cinco ejecuciones
válidas. De ellas, 59 alcanzaron la etapa de ejecución. Respecto de su estado
final, 54 metas quedaron abortadas, una fue completada y cinco no tenían estado
terminal en el último mensaje registrado. Estas cinco corresponden a la última
meta de cada ejecución, que el explorador canceló automáticamente al detenerse.
El registro se cerró antes de que esa cancelación terminara, por lo que la meta
figura en ejecución o en cancelación.

En este sistema, el estado abortado no equivale a un fallo de navegación. El
explorador vuelve a elegir su destino cada 3.3~s, período asociado a
\texttt{planner\_frequency} (Tabla~\ref{tab:explore_params_ch9}), y envía una
meta nueva cada vez que el punto de la frontera elegida cambia en 0.01~m o más,
sin cancelar antes la meta en curso. En la versión de Nav2 empleada, cuando
llega una meta nueva mientras otra está en ejecución, la anterior se termina con
estado abortado~\citep{nav2_foxy_src}. Por eso, el mismo estado registra tanto
los reemplazos hechos por el explorador como los fallos propios de Nav2.

Para separar ambos casos se revisó el orden de los mensajes de estado de cada
meta, según el procedimiento del \ref{anx:resultados_metodologia}. En 53 de los
54 abortos, la meta siguiente ya figuraba en ejecución mientras la meta abortada
seguía activa, es decir, la meta terminó porque el explorador la reemplazó. El
aborto restante corresponde a la primera meta de la iteración~1, que ya figuraba
abortada en el primer mensaje registrado y no pudo clasificarse. El tiempo
mediano que cada meta reemplazada permaneció activa fue de 3.3~s en las cinco
ejecuciones, igual al período de reevaluación del explorador
(Tabla~\ref{tab:anx_explore_metas}). Con los datos registrados no se identificó
ningún aborto originado por un fallo de Nav2. Tampoco es posible saber si las
metas reemplazadas habrían llegado a completarse.

En consecuencia, la proporción de metas completadas, 1 de 60 (1.7\%), indica
cuántas veces el robot llegó a menos de 0.10~m del punto de una frontera antes
de que el explorador cambiara de destino. No corresponde a una tasa de éxito de
la navegación ni de la exploración. La navegación hacia metas fijas, que
permitiría medir esa tasa, queda fuera del alcance declarado.

Los diecisiete avisos de lista de fronteras agotada registrados en las cinco
iteraciones válidas, once de ellos en la iteración~1, se relacionan con este
mismo comportamiento. El explorador agrega a su lista negra el punto de toda
meta que termina abortada, también cuando fue él quien la reemplazó. Así puede
descartar la frontera que estaba siguiendo y, cuando todas las candidatas quedan
en la lista, la vacía para volver a intentarlo. Este efecto de la integración
entre \texttt{explore\_lite} y Nav2 no impidió terminar la exploración, pero su
efecto sobre la duración del recorrido no se cuantificó. En la Fig.~\ref{fig:nav_exploracion_real} se muestran dos instantes representativos de una ejecución de navegación y exploración sobre hardware real, desde un mapa parcial hasta la mayor cobertura alcanzada.
...
radio de inflación de 0.15~m aplicado en el recinto reducido
(Sec.~\ref{sec:costmaps_ch9}) no se evaluó por separado, por lo que su efecto
sobre la navegación no se cuantificó. En conjunto, los ensayos verifican la integración funcional, cuantifican la
cobertura y documentan el resultado de las metas de exploración según la
Tabla~\ref{tab:anx_explore_metas}. Las cinco iteraciones finalizaron de forma
automática al no quedar fronteras, y cada una generó un mapa válido. La navegación dirigida con metas fijas definidas por el operador, distinta de
...
Control de velocidad & Evaluado experimentalmente & Indicadores de la Tabla~\ref{tab:pid_performance} calculados sobre la captura real en lazo cerrado a 0.13~m/s \\
...
Nav2 & Verificada funcionalmente & Ejecutó 59 metas del explorador. En 53 de los 54 abortos la meta fue reemplazada por el explorador (Tabla~\ref{tab:anx_explore_metas}), por lo que el resultado de las metas no mide su tasa de éxito. El protocolo con metas fijas queda fuera del alcance declarado (\hyperref[sec:alcances]{Alcances y limitaciones}) \\
Exploración autónoma & Verificada funcionalmente en hardware real &
Cinco de seis ejecuciones terminaron automáticamente al no quedar fronteras,
sin intervención manual y con un mapa válido. Cobertura libre media de 62.2\% y
área libre mapeada media de 3.50~m$^2$ \\
Localización & Resuelta por \texttt{slam\_toolbox} durante el mapeo en línea y
verificada funcionalmente en las cinco iteraciones válidas &
La localización sobre un mapa almacenado con AMCL queda fuera del alcance
declarado (\hyperref[sec:alcances]{Alcances y limitaciones}) y no se ensayó \\
\bottomrule
\end{tabular}
\end{table}
...
En la trayectoria rectilínea de 2~m, las desviaciones de la
Tabla~\ref{tab:resumen_odometria} no miden la misma magnitud en ambos entornos.
La de simulación se calcula con la propia odometría y la de hardware real con
la distancia física medida, de modo que el error odométrico solo puede
separarse del error de seguimiento en hardware real. En el protocolo angular,
la desviación media respecto de la referencia nominal es mayor en simulación
que en hardware real, con una dispersión menor
(Tabla~\ref{tab:resumen_giro}).

El mapeo y la exploración se evaluaron únicamente sobre hardware real, por lo
que no admiten un contraste directo entre simulación y hardware. Las
limitaciones metodológicas de cada ensayo se detallan en el
\ref{anx:resultados_metodologia}. Por su parte, la caracterización cuantitativa del consumo energético y de la carga computacional no forma parte de los ensayos realizados y se plantea como trabajo futuro; su alcance se precisa en el \ref{anx:resultados_metodologia}.
