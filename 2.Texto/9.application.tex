...
\State Lista negra $B\gets\varnothing$;\quad meta actual $g_{\mathrm{act}}\gets\textsc{nula}$;\quad $t_{\mathrm{ref}}\gets t_{\mathrm{now}}$
...
\While{el nodo de exploración está activo}
    ...
    \If{no existe $F^{*}$}
        ...
        \State \textbf{break}
    \EndIf
    \If{$F^{*}=g_{\mathrm{act}}$ \textbf{y} $t_{\mathrm{now}}-t_{\mathrm{ref}}>t_{\mathrm{prog}}$}
        \State $B\gets B\cup\{\text{punto de }g_{\mathrm{act}}\}$ \Comment{sin progreso: vetar y reseleccionar}
    \ElsIf{$F^{*}\neq g_{\mathrm{act}}$}
        \State Enviar \texttt{NavigateToPose}(centroide de $F^{*}$);\quad $g_{\mathrm{act}}\gets F^{*}$;\quad $t_{\mathrm{ref}}\gets t_{\mathrm{now}}$
    \EndIf
    \If{Nav2 devuelve \texttt{ABORTED} o meta rechazada}
        \State $B\gets B\cup\{\text{punto de }g_{\mathrm{act}}\}$ \Comment{frontera inalcanzable}
    \EndIf
\EndWhile
