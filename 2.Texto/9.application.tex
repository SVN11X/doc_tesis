...
Para mapear entornos desconocidos sin intervención manual continua, se integra un módulo de exploración basado en fronteras. Se utiliza el módulo de exploración por fronteras seleccionado en la Sec.~\ref{sec:exploracion} (\texttt{m-explore-ros2}); su nodo \texttt{explore\_server} identifica fronteras entre espacio libre conocido y zonas no observadas, y envía metas de navegación a Nav2.
...
\begin{algorithm}[H]
\caption{Ciclo de exploración por fronteras del explorador configurado (\texttt{m-explore-ros2}, adaptación a \gls{ROS2} de \texttt{explore\_lite})}
