...
Un simulador robótico integra un motor de física, la simulación de sensores y un entorno tridimensional configurable, lo que permite verificar la geometría, la cinemática y la lógica de control antes de operar el hardware. Gazebo, el simulador empleado, se integra de forma nativa con \gls{ROS2}: el modelo URDF se carga en el mundo simulado y los sensores publican datos con las mismas interfaces que sus contrapartes físicas \citep{koenig2004gazebo,openrobotics2024gazebo}. La comparación entre simuladores y la justificación de esta elección se presentan en la Sec.~\ref{sec:gemelo_digital}. La brecha simulación--realidad, es decir fricción, zonas muertas, ruido sensorial y latencias, se considera al interpretar los resultados del Capítulo~\ref{ch:resultados}.
...
La capa de transporte de \gls{ROS2} se basa en el estándar DDS (\textit{Data Distribution Service}), un middleware descentralizado que descubre los nodos de la red y negocia políticas de calidad de servicio \citep{macenski2022ros2}. Gracias a ello los nodos pueden distribuirse entre varios computadores de una red local sin un maestro central, como ocurre en este trabajo entre la Raspberry Pi~4 y la estación de trabajo. Además de los tópicos, \gls{ROS2} ofrece servicios, de solicitud y respuesta, y acciones, para tareas de larga duración con retroalimentación y cancelación, usadas por ejemplo para enviar metas de navegación. El árbol de transformaciones mantiene las relaciones espaciales entre los marcos de la Sec.~\ref{sec:robot_definicion}.
...
Un \gls{LIDAR} mide distancias emitiendo pulsos de luz láser y midiendo la señal reflejada, por tiempo de vuelo o por triangulación óptica \citep{ismail2022eslam}. En un \gls{LIDAR} 2D, el emisor y receptor giran sobre un eje vertical y entregan, en cada revolución, un barrido de pares ángulo y distancia en un plano horizontal, con el que se detectan paredes y obstáculos y se alimentan el SLAM y los mapas de costos. Sus limitaciones principales son la incapacidad de detectar objetos fuera del plano de barrido y la degradación ante superficies muy absorbentes o reflectantes. El sensor empleado, RPLidar A1, opera por triangulación con alcance nominal de 12~m y cobertura de 360$^\circ$ \citep{SlamtecA1Datasheet}.
...
El costo acumulado $g(\alpha)$ de una celda sucesora $\alpha$ contigua a $n$ se
obtiene sumando a $g(n)$ el costo de transitar hasta $\alpha$, que combina la
distancia recorrida y el costo de ocupación que el mapa asigna a la celda de
destino, mayor cerca de los obstáculos por la zona de inflación. Como heurística
$h(n)$ se emplea la distancia euclidiana de la celda a la meta, que es admisible
porque no sobreestima el costo real restante:
