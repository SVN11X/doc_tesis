...
Esa parametrización se consigue con \texttt{xacro}, un lenguaje de macros
para XML que permite definir constantes, reutilizar bloques y dividir la
...
Un \gls{LIDAR} mide distancias emitiendo pulsos de luz láser y midiendo el tiempo que tarda en volver la señal reflejada \citep[Sec.~6.8]{corke2023}. Los \gls{LIDAR} 2D también pueden medir por triangulación óptica \citep{ran2025review2dlidar}. En un \gls{LIDAR} 2D, el emisor y el receptor giran sobre un eje vertical y entregan, en cada revolución, un barrido de pares ángulo y distancia en un plano horizontal, con el que se detectan paredes y obstáculos y se alimentan el SLAM y los mapas de costos \citep{corke2023}. Sus limitaciones principales son la incapacidad de detectar objetos fuera del plano de barrido y la degradación ante superficies muy absorbentes o reflectantes. El sensor empleado, RPLidar A1, opera por triangulación y cubre 360$^\circ$ \citep{SlamtecA1Datasheet}.
...
sus ruedas \citep{ferreira2023differential}. Su cálculo es sencillo, lo que la hace
adecuada para sistemas embebidos. Sin embargo, acumula deriva por errores
sistemáticos, como un radio de rueda mal calibrado, y por errores aleatorios, como
el deslizamiento entre la rueda y el suelo \citep[Sec.~6.1]{corke2023}.
...
El controlador de velocidad se ejecuta de forma independiente para cada rueda en el Arduino Nano. La forma continua del PID, tomada de la formulación clásica de control realimentado \citep[Sec.~3.2]{astrom2009controlpid}, solo se utiliza como referencia conceptual:
...
trabajo, \citet{gokcce2021particle} se utiliza como antecedente de la
aplicación de PSO a la sintonización de controladores PID en robótica móvil,
pero no como fuente de la ecuación específica implementada. El script empleado
combina el factor de constricción analizado por \citet{clerc2002particle} con
un peso de inercia, mecanismo introducido en PSO por
\citet{shi1998modified}. La combinación concreta utilizada aquí, incluido el
factor global de constricción y la ley lineal del peso de inercia, corresponde
a la implementación ejecutada en este trabajo.

Cada partícula $i$ representa un candidato de solución, en este caso, un
...
las tres ganancias. En cada generación $g$, la velocidad de cada partícula se
actualiza combinando la velocidad previa, la atracción hacia su mejor posición
personal y la atracción hacia la mejor posición global del enjambre:
...
$[0,1]$ y $\zeta$ el factor de constricción. Los valores utilizados para
$c_1$, $c_2$ y $\zeta$ se consolidan en la Tabla~\ref{tab:pso_config}. El
empleo conjunto de constricción y peso de inercia en esta ecuación se
documenta como una variante implementada en el proyecto y no se atribuye
literalmente a una sola de las fuentes citadas.

El peso de inercia decrece linealmente con las generaciones:
...
\noindent donde $g_{\max}$ es el número máximo de generaciones. La disminución
lineal del peso de inercia fue propuesta por \citet{shi1998modified}, que lo
redujeron desde 1.4 hasta 0. El script usa esa misma forma, pero partiendo de 1. Su efecto es reducir progresivamente el peso de la
velocidad previa a medida que avanza la búsqueda. La función de costo empleada
para la sintonización combina el error cuadrático medio de la velocidad
respecto de la referencia con una penalización por sobreimpulso, favoreciendo
respuestas rápidas pero amortiguadas. Las escalas de búsqueda, las cotas, la
configuración del enjambre y las particularidades numéricas de la ejecución se
presentan en el Capítulo~\ref{ch:implementacion} y en el
\ref{anx:pso_detalle}.
...
Cada partícula representa una hipótesis de pose. Las hipótesis se propagan con la odometría y se ponderan según la concordancia entre los barridos del \gls{LIDAR} y el mapa de ocupación almacenado, y luego el remuestreo concentra las partículas en las regiones de mayor probabilidad. Este método estima la transformación entre el marco del mapa y el odométrico, mientras que la odometría mantiene la transformación entre este último y la base del robot. A diferencia del SLAM de la Sec.~\ref{sec:slam}, la localización no construye ni modifica el mapa, sino que sitúa al robot dentro de uno ya conocido. El uso de este método en la pila de navegación empleada y su relación con el
modo de localización utilizado en este trabajo se precisan en la
Sec.~\ref{sec:amcl_ch9}.
...
o desconocido \citep[Sec.~V]{macenski2023desks}. Sobre esa rejilla se construyen mapas de costo que agregan a los obstáculos una zona de inflación que penaliza
...
los nodos mediante la función de evaluación~\citep[Sec.~2.2.2]{lavalle2006planning}:
...
cada candidato y las evalúa mediante una suma ponderada de criterios~\citep[Sec.~III-A]{macenski2023desks}:
