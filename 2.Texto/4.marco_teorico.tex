...
\begin{algorithmic}[1]
\State \textbf{Archivo raíz} (\texttt{robot.urdf.xacro}): argumentos e inclusión de los módulos.
\State \textbf{Núcleo mecánico} (\texttt{robot\_core.xacro}): eslabones, articulaciones e inercias.
\State \textbf{Sensor láser} (\texttt{lidar.xacro}): eslabón y montaje del LIDAR.
\State \textbf{Interfaz de control} (\texttt{ros2\_control.xacro}): interfaces de comando y estado de las ruedas.
\end{algorithmic}
\end{algorithm}
...
La Fig.~\ref{fig:esquema_nodos} ilustra este patrón: uno o más nodos publican mensajes en un tópico y otros nodos se suscriben a él para recibirlos, sin que publicadores y suscriptores se conozcan entre sí.
...
la meta; $f(n)$ es el costo total estimado de la ruta que pasa por $n$.

El costo acumulado se actualiza de forma incremental: al expandir el nodo
$n$ hacia una celda sucesora $\alpha$ (contigua a $n$),
\begin{equation}
g(\alpha)=g(n)+c(n,\alpha),
\label{eq:astar_g}
\end{equation}

\noindent donde $c(n,\alpha)$ es el costo de transitar de la celda $n$ a la celda
sucesora $\alpha$, que combina la distancia geométrica recorrida y el costo de
ocupación que el mapa de costo asigna a la celda de destino (mayor cerca de
los obstáculos por la zona de inflación).

Como heurística se emplea una distancia geométrica al objetivo, habitualmente
la euclidiana:
\begin{equation}
h(n)=\lVert p(n)-p_{\mathrm{meta}}\rVert_{2},
\label{eq:astar_h}
\end{equation}

\noindent con $p(n)$ y $p_{\mathrm{meta}}$ las posiciones de la celda $n$ y de
la meta. Esta heurística es admisible, pues cumple
\begin{equation}
h(n)\le h^{*}(n),
\label{eq:astar_admisible}
\end{equation}

\noindent donde $h^{*}(n)$ es el costo real mínimo restante hasta la meta. Al
no sobreestimarlo, A* encuentra la ruta de costo mínimo expandiendo menos
nodos que una búsqueda sin información, lo que lo hace adecuado para
replanificar periódicamente sobre mapas de costo.
