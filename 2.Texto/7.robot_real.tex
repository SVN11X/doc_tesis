...
En \texttt{write()}, la velocidad angular de rueda comandada por \texttt{diff\_drive\_controller} se traduce al formato de \textit{ticks/frame} esperado por el \gls{Arduino}:

\begin{equation}
m_j^{cmd}[k] = \frac{\omega_j^{cmd}[k]}{2\pi/N_e}\cdot\frac{1}{f_{ctrl}}
= \frac{\omega_j^{cmd}[k]\,N_e}{2\pi\,f_{ctrl}},
\qquad j\in\{R,L\}
\label{eq:cmd_to_ticks}
\end{equation}

donde $m_j^{cmd}[k]$ es la referencia enviada al firmware [ticks/frame], $\omega_j^{cmd}[k]$ la velocidad angular de rueda comandada [rad/s], $N_e=1980$ la resolución del encoder [ticks/vuelta] y $f_{ctrl}=30$~Hz la frecuencia común del lazo (\texttt{loop\_rate} configurada igual a \texttt{PID\_RATE}). El término $2\pi/N_e$ corresponde a la constante \texttt{rads\_per\_count}. Esta conversión es equivalente a la Ec.~\ref{eq:mps_to_ticks}, ya que $v_j^{cmd}=r\,\omega_j^{cmd}$.
...
\item \texttt{DiffDriveArduino} traduce $\omega_j^{cmd}$ a \textit{ticks/frame} (Ec.~\ref{eq:cmd_to_ticks}) y los envía al \gls{Arduino}.
...
\If{el \gls{Arduino} no está conectado} \State \Return ERROR \EndIf
\For{$j\in\{R,L\}$}
\State $m_j^{cmd}[k]\gets \omega_j^{cmd}[k]\,N_e/(2\pi f_{ctrl})$ \Comment{Ec.~\ref{eq:cmd_to_ticks}}
\EndFor
