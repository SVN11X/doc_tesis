...
\qquad
\dot{\phi}_j[k] = \frac{\phi_j[k]-\phi_j[k-1]}{\Delta t_k},
\qquad j\in\{R,L\}
\label{eq:enc_to_state}
\end{equation}

donde $\phi_j[k]$ es el ángulo acumulado de la rueda $j$ [rad], $c_j[k]$ las cuentas acumuladas del encoder [cuentas], $\dot{\phi}_j[k]$ la velocidad angular estimada de la rueda [rad/s] y $\Delta t_k$ el período real medido entre lecturas consecutivas [s]; el resto de variables fueron definidas previamente. El uso de $\Delta t_k$ medido, en lugar del período nominal $1/f_{ctrl}$, evita sesgos cuando el ciclo no se ejecuta exactamente a la frecuencia teórica.

La velocidad $\dot{\phi}_j[k]$ es la misma magnitud que $\omega_j$ (Ec.~\ref{eq:w_ruedas}); difieren solo en la base temporal: $\omega_j$ usa el período nominal $T_s$ y $\dot{\phi}_j$ el período medido $\Delta t_k$.
...
\item El plugin solicita periódicamente los encoders y reconstruye $\phi_j$ y $\dot{\phi}_j$ (Ec.~\ref{eq:enc_to_state}).
...
\Ensure Estados de rueda $(\phi_j,\dot{\phi}_j)$ para odometría y referencias \textit{ticks/frame} enviadas al firmware.
...
\While{el controlador está activo}
    ...
    \For{$j\in\{R,L\}$}
        \State $\phi_j[k]\gets c_j[k]\cdot(2\pi/N_e)$; \quad $\dot{\phi}_j[k]\gets(\phi_j[k]-\phi_j[k-1])/\Delta t_k$ \Comment{Ec.~\ref{eq:enc_to_state}}
    \EndFor
    \State Exportar $\phi_j[k],\dot{\phi}_j[k]$ como \textit{state interfaces}
    \Statex
