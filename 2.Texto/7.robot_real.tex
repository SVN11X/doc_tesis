...
\texttt{update\_rate} & 30~Hz & Frecuencia del controlador ($f_{loop}$) \\
\texttt{enable\_odom\_tf} & true & Publica \texttt{odom}$\rightarrow$\texttt{base\_link} \\
\bottomrule
\end{tabular}
\end{table}

La comunicación con el firmware se realiza mediante el plugin \texttt{DiffDriveArduino}, una interfaz de sistema de \texttt{ros2\_control} que opera en un ciclo periódico \texttt{read()} $\rightarrow$ \texttt{update()} $\rightarrow$ \texttt{write()} a la frecuencia $f_{loop}$ del \texttt{controller\_manager}.
...
\begin{equation}
m_j^{cmd}[k] = \frac{\omega_j^{cmd}[k]}{2\pi/N_e}\cdot\frac{1}{f_{loop}}
= \frac{\omega_j^{cmd}[k]\,N_e}{2\pi\,f_{loop}},
\qquad j\in\{R,L\}
\label{eq:cmd_to_ticks}
\end{equation}

donde $m_j^{cmd}[k]$ es la referencia enviada al firmware [ticks/frame], $\omega_j^{cmd}[k]$ la velocidad angular de rueda comandada [rad/s], $N_e=1980$ la resolución del encoder [cuentas/vuelta] y $f_{loop}=30$ Hz la frecuencia del lazo del controlador (\texttt{loop\_rate}). El término $2\pi/N_e$ corresponde a la constante \texttt{rads\_per\_count} de cada rueda. Esta conversión es equivalente a la del Capítulo~\ref{marco_teorico} expresada en velocidad lineal, ya que $v_j=r\,\omega_j$.
...
\begin{algorithmic}[1]
\Require Comandos de velocidad angular de rueda $\omega_R^{cmd},\omega_L^{cmd}$ [rad/s] (de \texttt{diff\_drive\_controller}); enlace serial al \gls{Arduino}; $N_e$; $f_{loop}$; ganancias PID residentes en el firmware.
...
\Statex
\State \textbf{Ejecución principal} (cada período del \texttt{controller\_manager}, a $f_{loop}$):
\While{el controlador está activo}
    ...
    \If{el \gls{Arduino} no está conectado} \State \Return ERROR \EndIf
    \For{$j\in\{R,L\}$}
        \State $m_j^{cmd}[k]\gets \omega_j^{cmd}[k]\,N_e/(2\pi f_{loop})$ \Comment{Ec.~\ref{eq:cmd_to_ticks}}
    \EndFor
    \State Enviar $m_R^{cmd}[k],m_L^{cmd}[k]$ al \gls{Arduino}
\EndWhile

\Statex
\State \textbf{Salida:} El firmware ejecuta el PID por rueda a $f_{PID}$ y aplica el \textit{watchdog} de auto-detención; el controlador publica \texttt{/odom} y la transformación \texttt{odom}$\rightarrow$\texttt{base\_link}.
\end{algorithmic}
\end{algorithm}
