...
donde $m_j^{cmd}[k]$ es la referencia enviada al firmware [ticks/frame], $\omega_j^{cmd}[k]$ la velocidad angular de rueda comandada [rad/s], $N_e=1980$ la resolución del encoder [cuentas/vuelta] y $f_{ctrl}=30$~Hz la frecuencia común del lazo (\texttt{loop\_rate} configurada igual a \texttt{PID\_RATE}). El término $2\pi/N_e$ corresponde a la constante \texttt{rads\_per\_count}. Esta conversión es equivalente a la Ec.~\ref{eq:mps_to_ticks}, ya que $v_j^{cmd}=r\,\omega_j^{cmd}$.

En \texttt{read()}, los conteos acumulados del encoder se transforman en ángulo y velocidad de rueda, que alimentan la odometría:
