...
\For{$j\in\{R,L\}$}
\State Calcular $\phi_j[k]$ y $\hat{\omega}_j[k]$ mediante la Ec.~\ref{eq:enc_to_state}.
\EndFor
...
\If{el \gls{Arduino} no está conectado} \State \Return ERROR \EndIf
\For{$j\in\{R,L\}$}
\State Calcular $m_j^{cmd}[k]$ mediante la Ec.~\ref{eq:mps_to_ticks}, usando $v_j^{cmd}=r\,\omega_j^{cmd}$.
\EndFor
